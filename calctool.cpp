...
void CalcTool::spiltText(QString &newText, std::array<QString, 3> &arr) {
    ...
    if (loc == -1) {
        ...
        if (newText.at(loc - 1) == op) {
            arr[0] = newText.left(loc - 1);
        } else {
...
void CalcTool::calculateOutput() {
    ...
    updating = true;
    QChar opt = '+';
    QString num1 = decOriTextArr[0];
    QString num2 = "";
    if(decOriTextArr[1] == "") {
        num2 = "0";
    } else {
        opt = decOriTextArr[1].at(0);
        num2 = decOriTextArr[2];
    }
...
void CalcTool::formatOutput(bool outputHistory) {
    ...
    if(outputHistory) {
        ...
        history += "================\r\n";
        ui->HistoryText->insertPlainText(history);
    }
